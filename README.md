# 🤖 MOEX Trading Bot

Торговый бот для MOEX с использованием уровней Донченнела и интеграцией с Telegram.

## 📋 Содержание

- [Возможности](#возможности)
- [Требования](#требования)
- [Установка](#установка)
- [Конфигурация](#конфигурация)
- [Запуск](#запуск)
- [Docker](#docker)
- [Разработка](#разработка)
- [Деплой](#деплой)
- [Мониторинг](#мониторинг)
- [Troubleshooting](#troubleshooting)

## ✨ Возможности

- 📊 Анализ уровней Донченнела для акций MOEX
- 🤖 Telegram бот для управления и уведомлений
- 📈 Симуляция торговли (бэктестинг)
- 🔄 Инкрементальная загрузка исторических данных
- ⚡ Автоматическое отслеживание ордеров
- 🛡️ Graceful shutdown и обработка ошибок
- 📝 Структурированное логирование
- 🐳 Docker ready
- 🔍 Health checks

## 📦 Требования

- Node.js >= 20.0.0
- npm >= 9.0.0
- Docker (опционально)
- PM2 (опционально, для продакшен деплоя)

## 🚀 Установка

### 1. Клонирование репозитория

```bash
git clone <repository-url>
cd moex-donchian-levels
```

### 2. Установка зависимостей

```bash
npm install
```

### 3. Настройка окружения

```bash
cp .env.example .env
```

Отредактируйте `.env` файл и укажите свои credentials:

```env
TG_BOT_TOKEN=your_telegram_bot_token
TG_CHAT_ID=your_chat_id
ALOR_REFRESH_TOKEN=your_alor_token
ALOR_CLIENT=your_client_id
ALOR_PORTFOLIO=your_portfolio
```

## ⚙️ Конфигурация

### Переменные окружения

| Переменная | Описание | По умолчанию |
|------------|----------|--------------|
| `TG_BOT_TOKEN` | Токен Telegram бота | - |
| `TG_CHAT_ID` | ID чата для уведомлений | - |
| `ALOR_REFRESH_TOKEN` | Refresh token Alor API | - |
| `ALOR_CLIENT` | Client ID Alor | - |
| `ALOR_PORTFOLIO` | Portfolio ID Alor | - |
| `NODE_ENV` | Окружение | `development` |
| `PORT` | Порт для health check | `3000` |
| `LOG_LEVEL` | Уровень логирования | `info` |
| `MOCK_MODE` | Режим моков (без реального API) | `false` |
| `MOEX_API_TIMEOUT` | Таймаут MOEX API (мс) | `10000` |
| `ALOR_API_TIMEOUT` | Таймаут Alor API (мс) | `15000` |

## 🏃 Запуск

### Режим разработки

```bash
npm run dev
```

### Продакшн режим

```bash
npm run build
npm start
```

### Запуск бота

```bash
npm run bot
```

### Получение ордеров

```bash
npm run orders
```

## 🐳 Docker

### Сборка образа

```bash
npm run docker:build
```

### Запуск контейнера

```bash
npm run docker:up
```

### Остановка контейнера

```bash
npm run docker:down
```

### Просмотр логов

```bash
npm run docker:logs
```

### Docker Compose (полный стек)

```bash
docker-compose up -d
```

## 💻 Разработка

### Структура проекта

```
.
├── src/
│   ├── bot/                  # Telegram бот
│   │   ├── alor/            # Интеграция с Alor API
│   │   ├── history/         # Работа с историческими данными
│   │   ├── instruments/     # Инструменты и тикеры
│   │   └── sessions/        # Управление сессиями
│   ├── config/              # Конфигурация
│   ├── health/              # Health checks
│   ├── order-state/         # Состояние ордеров
│   ├── sim-trade/           # Симуляция торговли
│   └── utils/               # Утилиты
├── data/                     # Данные (не в git)
├── logs/                     # Логи (не в git)
├── Dockerfile               # Docker образ
├── docker-compose.yml       # Docker Compose
└── ecosystem.config.cjs     # PM2 конфигурация
```

### Скрипты разработки

```bash
# Сборка
npm run build

# Очистка dist
npm run clean

# Пересборка
npm run rebuild

# Проверка типов
npm run type-check

# Линтинг
npm run lint

# Форматирование
npm run format
```

## 🚀 Деплой

### Вариант 1: PM2 (рекомендуется)

```bash
# Установка PM2 глобально
npm install -g pm2

# Сборка проекта
npm run build

# Запуск через PM2
npm run pm2:start

# Остановка
npm run pm2:stop

# Перезапуск
npm run pm2:restart

# Просмотр логов
npm run pm2:logs

# Мониторинг
npm run pm2:monit
```

### Вариант 2: Systemd

Создайте файл `/etc/systemd/system/moex-bot.service`:

```ini
[Unit]
Description=MOEX Trading Bot
After=network.target

[Service]
Type=simple
User=bot
WorkingDirectory=/path/to/moex-donchian-levels
Environment=NODE_ENV=production
ExecStart=/usr/bin/node dist/bot/main.js
Restart=always
RestartSec=10
StandardOutput=journal
StandardError=journal

[Install]
WantedBy=multi-user.target
```

Затем:

```bash
sudo systemctl daemon-reload
sudo systemctl enable moex-bot
sudo systemctl start moex-bot
sudo systemctl status moex-bot
```

### Вариант 3: Docker

```bash
# Сборка и запуск
docker-compose up -d

# Просмотр статуса
docker-compose ps

# Логи
docker-compose logs -f bot
```

## 📊 Мониторинг

### Health Check Endpoints

Приложение предоставляет следующие эндпоинты:

- `GET /health` - Полная информация о состоянии
- `GET /ready` - Готовность к обработке запросов
- `GET /live` - Проверка жизнеспособности

Пример:

```bash
curl http://localhost:3000/health
```

Ответ:

```json
{
  "status": "ok",
  "timestamp": "2024-02-14T12:00:00.000Z",
  "uptime": 3600,
  "version": "2.0.0",
  "services": {
    "moex": "ok",
    "telegram": "ok",
    "alor": "ok"
  }
}
```

### Логи

Логи сохраняются в директории `logs/`:

- `application-YYYY-MM-DD.log` - Все логи
- `error-YYYY-MM-DD.log` - Только ошибки
- `pm2-out.log` - Stdout (при использовании PM2)
- `pm2-error.log` - Stderr (при использовании PM2)

Ротация логов происходит автоматически (хранятся 14 дней для application, 30 дней для error).

### PM2 Monitoring

```bash
# Dashboard
pm2 monit

# Список процессов
pm2 list

# Детальная информация
pm2 show moex-bot

# Логи реального времени
pm2 logs moex-bot
```

## 🔧 Troubleshooting

### Проблема: "timeout of 10000ms exceeded"

**Причина:** MOEX API не отвечает

**Решение:**
1. Проверьте системное время: `date`
2. Проверьте доступность MOEX API: `curl https://iss.moex.com/iss/index.json`
3. Временно включите MOCK режим: `MOCK_MODE=true npm start`

### Проблема: "Missing required environment variables"

**Причина:** Не настроен .env файл

**Решение:**
1. Скопируйте `.env.example` в `.env`
2. Заполните все обязательные переменные
3. Или включите `MOCK_MODE=true` для тестирования

### Проблема: Бот не отвечает в Telegram

**Причина:** Неверный токен или проблемы с сетью

**Решение:**
1. Проверьте `TG_BOT_TOKEN` в .env
2. Проверьте логи: `npm run pm2:logs` или `docker-compose logs -f`
3. Проверьте firewall и сетевые настройки

### Проблема: Контейнер постоянно перезапускается

**Причина:** Ошибка при запуске

**Решение:**
1. Проверьте логи: `docker-compose logs bot`
2. Проверьте health check: `docker-compose ps`
3. Убедитесь, что .env файл настроен правильно

## 📝 Changelog

### Version 2.0.0

- ✨ Добавлен Docker support
- ✨ Добавлены health check endpoints
- ✨ Улучшено логирование (Winston)
- ✨ Добавлен graceful shutdown
- ✨ Настроена конфигурация через .env
- 🐛 Исправлена проблема с форматом времени в historyIncremental
- 🐛 Улучшена обработка ошибок MOEX API
- 🔒 Секреты вынесены из репозитория

## 📄 License

MIT

## 🤝 Contributing

Pull requests are welcome!

## 📧 Support

Если у вас возникли проблемы, создайте issue в репозитории.
